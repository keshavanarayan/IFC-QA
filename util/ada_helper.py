...
def check_doors(ifc_file):
    ...
    units = get_project_units(ifc_file)[0]
    unitscale = get_project_units(ifc_file)[1]
    ...
    for door in doors:
        ...
        if door.OverallWidth < min_width:
            doors_minor.append([get_id(door),door.Name,f"Reduce width by {door.OverallWidth - min_width} {units}"])
            continue
        
        if not handicap_accessible:
            doors_major.append([get_id(door),door.Name,"Make Door with Handicap Friendly Materials"])
            continue
        
        else:
            doors_ok.append([get_id(door),door.Name,door.OverallWidth])
...
def check_floors(ifc_file):
    ...
    element_to_storey = element_wrt_storey(ifc_file)
